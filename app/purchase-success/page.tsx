...
export default async function PurchaseSuccessPage({
  ...
  const ticketId   = session.metadata?.ticketId ?? sessionId;
  const orderNumber = sessionId.slice(-12).toUpperCase();

  // Build QR payload — encodes ticket identity for venue scanning
    ...
    <div className="mx-auto max-w-lg px-4 py-16">
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden"
        ...
        <div className="bg-gradient-to-r from-[#059669] to-[#10B981] px-8 py-10 text-center">
          ...
          <p className="text-white/80 text-sm mt-2">Your ticket is secured.</p>
        </div>

        <div className="px-8 py-7 space-y-5">
          {/* Event */}
          <div className="bg-[#F8F9FA] rounded-xl p-5 border border-[#E2E8F0]">
            <p className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-2">Event</p>
            <p className="font-bold text-[#0F172A] text-lg leading-tight">{eventName}</p>
            {seatInfo && <p className="text-[#64748B] text-sm mt-1">{seatInfo}</p>}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-[#0F172A] font-semibold">{quantity} ticket{quantity !== 1 ? "s" : ""}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-[#F1F5F9] pt-3">
              <span className="text-[#64748B]">Total Paid</span>
              <span className="text-[#0F172A] font-bold text-base">{currency} ${totalUsd}</span>
            </div>
          </div>
          ...
          {buyerEmail && (
            <p className="text-xs text-[#94A3B8] text-center">
              Confirmation sent to: <span className="text-[#64748B] font-medium">{buyerEmail}</span>
            </p>
          )}

          {/* QR Code */}
          <div className="border-t border-[#F1F5F9] pt-5">
            <p className="text-xs font-semibold text-[#94A3B8] uppercase tracking-widest text-center mb-4">
              Your Entry QR Code
            </p>
            <div className="flex justify-center">
              <DownloadableQR data={qrPayload} label={orderNumber} size={180} />
            </div>
          ...
          <div className="space-y-3 pt-2">
            <Link href="/my-tickets"
                  className="btn-primary w-full py-3.5 text-sm font-bold rounded-xl flex items-center justify-center">
              View My Tickets
            </Link>
            <Link href="/"
                  className="block w-full py-3 text-sm font-semibold text-[#64748B] text-center
